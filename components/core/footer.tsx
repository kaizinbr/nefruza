import Link from "next/link";
import Image from "next/image";

import { LuInstagram, LuFacebook, LuMap, LuLinkedin } from "react-icons/lu";
import { PiPhoneCallFill } from "react-icons/pi";
import { TbMailFilled, TbBrandWhatsappFilled } from "react-icons/tb";

export default function Footer() {
    return (
        <footer className="flex w-full flex-col items-center justify-center border-t border-zinc-200">
            <div
                className={`
                    flex flex-wrap w-full  flex-col items-center justify-center
                    bg-nef-700 px-4 md:px-16 pt-12 text-zinc-200
                `}
            >
                <div
                    className={`
                        flex flex-wrap w-full max-w-7xl items-start justify-between pb-12
                         xl:gap-0
                    `}
                >
                    <div className="flex flex-col lg:flex-row items-start gap-8 lg:gap-12 xl:gap-18 w-full mb-8">
                        <div className="flex flex-col items-start gap-2">
                            <span className="font-bold font-title ext-sm uppercase">
                                Telefone fixo
                            </span>
                            <Link
                                href="tel:(83) 3225-1619"
                                className="text-xl xl:text-2xl font-bold  flex flex-row items-center gap-2 hover:text-nef-300 transition-colors"
                            >
                                <PiPhoneCallFill /> (83) 3225-1619
                            </Link>
                            <span className="text-xs text-zinc-300">
                                Segunda a sexta-feira, das 8h às 18h
                            </span>
                        </div>
                        <div className="flex flex-col items-start gap-2">
                            <span className="font-bold font-title text-sm  uppercase">
                                Telefone fixo
                            </span>
                            <Link
                                href="tel:(83) 3225-1985"
                                className="text-xl xl:text-2xl font-bold  flex flex-row items-center gap-2 hover:text-nef-300 transition-colors"
                            >
                                <PiPhoneCallFill /> (83) 3225-1985
                            </Link>
                            <span className="text-xs text-zinc-300">
                                Segunda a sexta-feira, das 8h às 18h
                            </span>
                        </div>
                        <div className="flex flex-col items-start gap-2">
                            <span className="font-bold font-title text-sm  uppercase">
                                WhatsApp Nefruza
                            </span>
                            <Link
                                href="https://wa.me/5583999452332?text=Ol%C3%A1%2C%20tudo%20bem%3F%20Vim%20pelo%20site%20da%20Nefruza."
                                target="_blank"
                                className="text-xl xl:text-2xl font-bold  flex flex-row items-center gap-2 hover:text-nef-300 transition-colors"
                            >
                                <TbBrandWhatsappFilled /> (83) 9 9945-2332
                            </Link>
                            <span className="text-xs text-zinc-300">
                                Segunda a sexta-feira, das 8h às 18h
                            </span>
                        </div>
                        <Link
                            href="/contato"
                            className="text-sm hover:text-nef-400 transition-colors py-3 px-6 my-auto rounded-full border border-nef-300 hover:border-nef-400"
                        >
                            Mais contatos
                        </Link>
                    </div>
                    <div className="flex flex-col lg:flex-row w-full justify-start gap-8 lg:gap-24 pt-8 border-t border-nef-400">
                        <div className="flex flex-col gap-2 md:w-1/3 lg:w-auto w-1/1">
                            <Link
                                href="/institucional"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 font-bold font-title uppercase transition-colors"
                            >
                                A Nefruza
                            </Link>
                            <Link
                                href="/institucional/historia"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Nossa história
                            </Link>
                            <Link
                                href="/institucional/estrutura"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Nossa estrutura
                            </Link>
                            <Link
                                href="/institucional/proposito-e-valores"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Propósito e valores
                            </Link>
                            <Link
                                href="/institucional#pioneirismo"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Pioneirismo
                            </Link>
                        </div>
                        <div className="flex flex-col gap-2 md:w-1/5 lg:w-auto w-1/1">
                            <Link
                                href="/"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 font-bold font-title uppercase transition-colors"
                            >
                                Site
                            </Link>
                            <Link
                                href="/"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Início
                            </Link>
                            <Link
                                href="/contato"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Contato
                            </Link>
                            <Link
                                href="/servicos"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Serviços
                            </Link>
                            <Link
                                href="/convenios"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Convênios
                            </Link>
                            <Link
                                href="/equipe"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Equipe
                            </Link>
                            <Link
                                href="/contato/ouvidoria"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Ouvidoria
                            </Link>
                        </div>
                        <div className="flex flex-col gap-2 md:w-1/5 lg:w-auto w-1/1">
                            <Link
                                href="/blog"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 font-bold uppercase font-title transition-colors"
                            >
                                Informativo
                            </Link>
                            <Link
                                href="/blog"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Blog
                            </Link>
                            <Link
                                href={{
                                    pathname: "/blog",
                                    query: { category: "noticias" },
                                }}
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Notícias
                            </Link>
                            <Link
                                href={{
                                    pathname: "/blog",
                                    query: { category: "comunicados" },
                                }}
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Comunicados
                            </Link>
                            <Link
                                href="/politica-de-privacidade"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Política de privacidade
                            </Link>
                            <Link
                                href="/termos-de-uso"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Termos de uso
                            </Link>
                            <Link
                                href="/codigo-de-etica"
                                rel="noopener noreferrer"
                                className=" hover:text-nef-300 transition-colors"
                            >
                                Código de ética
                            </Link>
                        </div>
                    </div>
                </div>
            </div>

            <div
                className={`
                    flex flex-wrap w-full  flex-col items-center justify-center
                    bg-nef-700 px-4 md:px-16
                `}
            >
                <div
                    className={`
                        flex flex-col lg:flex-row w-full max-w-7xl
                        items-center lg:items-start justify-center lg:justify-between py-8
                        gap-4 gap-y-8 lg:gap-0
                        border-t border-nef-400
                    `}
                >
                    <div className="flex flex-col items-center xl:items-start gap-2 w-auto">
                        <span className="text-sm font-bold text-zinc-50 uppercase font-title mb-2">
                            Siga nossas redes sociais
                        </span>
                        <div className="flex flex-row items-center gap-4 text-zinc-50 text-2xl">
                            <Link
                                href="https://www.instagram.com/nefruza"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="hover:text-nef-300 transition-colors"
                            >
                                <LuInstagram />
                            </Link>
                            <Link
                                href="https://www.facebook.com/nefruza"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="hover:text-nef-300 transition-colors"
                            >
                                <LuFacebook />
                            </Link>
                            <Link
                                href="https://www.linkedin.com/company/nefruza"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="hover:text-nef-300 transition-colors"
                            >
                                <LuLinkedin />
                            </Link>
                            <Link
                                href="https://maps.app.goo.gl/BSiF3bnGe8WeimKV8"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="hover:text-nef-300 transition-colors"
                            >
                                <LuMap />
                            </Link>
                        </div>
                    </div>
                    <div className="flex flex-col items-center text-center gap-2 w-auto">
                        <span className="text-sm text-zinc-50 ">
                            © 2026 Nefruza Serviços Nefrológicos Fiúza Chaves
                        </span>
                        <span className="text-sm text-zinc-50 ">
                            CNPJ 09.291.683/0001-58 | RT: Dra.
                            Erika Fiúza - CRM/PB 6596
                        </span>
                        <span className="text-sm text-zinc-50">
                            Av. Sinésio Guimarães, 290 Torre, João Pessoa - PB,
                            CEP 58040-400
                        </span>
                    </div>
                    <div className="flex flex-col items-end gap-2 w-auto">
                        <Image
                            src="/img/logo.webp"
                            alt="Nefruza Logo"
                            width={164}
                            height={32}
                            className="invert"
                        />
                    </div>
                </div>
                {/* <div className="flex flex-col gap-2 md:flex-row w-full max-w-7xl items-start justify-between py-4 border-t border-nef-400">
                    <span className="text-sm text-zinc-50 ">
                        © 2026 Nefruza Serviços Nefrológicos Fiúza Chaves Ltda.
                        Todos os direitos reservados.
                    </span>
                    <span className="text-sm text-zinc-50 ">
                        CNPJ 09.291.683/0001-58 | Responsável Técnica: Dra.
                        Erika Fiúza - CRM/PB 6596
                    </span>
                </div> */}
            </div>
        </footer>
    );
}
